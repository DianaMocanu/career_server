...
exports.getById = async (req, res) => {
  try {
    ...
    const users = await db.get(
      'SELECT users.username as username, users.email, games.username as gamename, scores.points FROM users, games, scores where scores.userId = $id and scores.gameId = games.id',
      { $id: id },
    );
...
exports.post = async (req, res, next) => {
  try {
    ...
    if (!username || !email || !password) return res.status(400).send('Bad request/AddUser');

    await db
      .run(`INSERT INTO users(username, email, password) VALUES ($name, $email, $password)`, {
        $name: username,
        $email: email,
        $password: password,
      })
